...
def send_smtp_report(report: dict, recipients: Optional[list[str]] = None, mailbox: Optional[dict] = None) -> bool:
    ...
    subject = (
        f"[Attijari] {report['period'].title()} Report — "
...
def _format_html_report(report: dict) -> str:
    ...
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color: #1a1a2e; padding: 20px;">
        <h2 style="color: #0f3460;">Attijari — {report['period'].title()} Report</h2>
        ...
        <p style="color:#6b7280; font-size:12px; margin-top:20px;">
            Generated by Attijari Triage System at {report['generated_at'][:19]} UTC
        </p>
    </body>
    </html>
    """
...
def send_slack_report(report: dict, webhook_url: Optional[str] = None) -> bool:
    ...
    blocks = [
        {
            "type": "header",
            "text": {"type": "plain_text", "text": f"📊 Attijari {report['period'].title()} Report"}
        },
        {
            "type": "section",
            "fields": [
        ...
        "attachments": [{
            ...
            "content": {
                ...
                "body": [
                    {
                        "type": "TextBlock",
                        "text": f"📊 Attijari {report['period'].title()} Report",
                        "size": "Large",
                        "weight": "Bolder",
                    },
                    {
